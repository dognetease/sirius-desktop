...
var Entry = function Entry(_ref) {
  ...
  var onMouseEnter = function onMouseEnter() {
    onMentionFocus(index, "mouseenter");
  };
      ...
      return mentionTriggers
        .map(function(trigger) {
          return (
            ...
            (mentionTriggers.length === 1 && anchorOffset >= start + trigger.length && anchorOffset <= end) ?
            trigger :
            undefined
          );
        })
...
var MentionSuggestions = /*#__PURE__*/ (function(_Component) {
  ...
  function MentionSuggestions(props) {
    ...
    _this.onMentionFocus = function(index, eType) {
      // 键盘事件触发的滚动导致的mouseenter事件，不做响应（通过与上一次pageY比较行不通，暂时用这种方法）
      if (eType === "mouseenter" && _this.state.keyboardOperationing) {
        return;
      }
...
function findInContentBlock(regex, text, nonEntityStart, callback) {
  ...
  while ((matchArr = regex.exec(text)) !== null) {
    ...
    // @autor:郭超
    // 源代码 whitespaceRegEx.test(text[start])
    /**
     * @author:郭超
     * @问题场景：@前输入一个中文字符之后光标没有停留到中文位置,会向后移动一位
     * @原代码:whitespaceRegEx.test(text[start])
     * 修改原因:因为regex修改了 不在检测@前有没有空格。如果不修改whitespaceRegEx和test规则
     * 会导致callback截取的区间有问题(多包涵了一个@前的字符)
     *
     */
    if (whitespaceRegEx.test(text.slice(start))) {
...
function findInContentBlockWithWhitespace(regex, text, nonEntityStart, callback) {
  ...
  while ((matchArr = regex.exec(text)) !== null) {
    ...
    console.log("[draft]plugin:findInContentBlockWithWhitespace", start, _end2, matchArr);
...
export { MentionSuggestions$1 as MentionSuggestions, addMention, index as default, defaultSuggestionsFilter, defaultTheme };