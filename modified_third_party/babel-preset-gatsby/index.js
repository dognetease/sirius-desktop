...
function preset(_, options = {}) {
  let {
    targets = null,
  } = options; // TODO(v3): Remove process.env.GATSBY_BUILD_STAGE, needs to be passed as an option

  const stage = options.stage || process.env.GATSBY_BUILD_STAGE || `test`;
  const pluginBabelConfig = loadCachedConfig();
  let isBrowser; // unused because of cloud builds
  // const absoluteRuntimePath = path.dirname(
  //   require.resolve(`@babel/runtime/package.json`)
  // )

  if (!targets) {
    if (stage === `build-html` || stage === `develop-html` || stage === `test`) {
      targets = {
        node: `current`,
      };
    } else {
    ...
    presets: [
      [
        ...
        exclude: [// Exclude transforms that make all code slower (https://github.com/facebook/create-react-app/pull/5278)
          `transform-typeof-symbol`, // we already have transforms for these
          `transform-spread`, `proposal-nullish-coalescing-operator`, `proposal-optional-chaining`, ...polyfillsToExclude,
        ],
      },
      ], [
        resolve(`@babel/preset-react`), {
          useBuiltIns: true,
          pragma: options.reactRuntime === `automatic` ? undefined : `React.createElement`,
    ...
    plugins: [
      ...
      [
        resolve(`@babel/plugin-transform-runtime`),
        {
          ...
          helpers: true,
          regenerator: true,
          useESModules: stage !== `test`, // this breaks cloud builds so we disable it for now
      ...
      stage === `build-javascript` && [// Remove PropTypes from production build
        resolve(`babel-plugin-transform-react-remove-prop-types`), {
          removeImport: true,
        },
      ],
      !IS_TEST && [resolve(`babel-plugin-transform-remove-console`)],
    ].filter(Boolean),
  };
}