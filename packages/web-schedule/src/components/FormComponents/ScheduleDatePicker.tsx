...
const customHeader: ReactDatePickerProps<any>['renderCustomHeader'] = ({
  ...
  <div className={`${styles.header} schedule-date-picker-header`}>
    <i onClick={decreaseMonth} className={classNames(styles.arrow, 'arrow', styles.left, 'left')} />
    <span className={styles.dater}>{moment(date).format(getIn18Text('yyyyNIAN'))}</span>
    <i onClick={increaseMonth} className={classNames(styles.arrow, 'arrow', styles.right, 'right')} />
  </div>
);
