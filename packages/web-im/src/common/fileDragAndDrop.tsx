...
export const FilesDragAndDrop: React.FC<FilesDragAndDropApi> = props => {
  const {
    onUpload,
    placeholder = getIn18Text('SONGSHOUSHANGCHUAN'),
    count = 100,
    excludes = [],
    ...
    dragClassname = '',
    dropClassname = '',
    dropComponent = <p className={realStyle('')}>{getIn18Text('SONGSHOUCHUFASHANG')}</p>,
  } = props;
  ...
  const _validates = [
    async (files: File[]) => {
      ...
      for (let i = 0; i < len; i++) {
        try {
          //@ts-ignore
          if (Blob && Blob.prototype && Blob.prototype.arrayBuffer) {
            await files[i].slice(0, 1).arrayBuffer();
          } else {
            const size = files[i].slice(0, 1).size;
            if (size === 0) {
              flag = true;
            }
          }
  ...
  useEffect(() => {
    const node = drag.current as unknown as HTMLElement;
  ...
  useEffect(() => {
    ...
    const dropNode = drop.current as unknown as HTMLElement;
    const node = drag.current as unknown as HTMLElement;
  ...
  return (
    ...
    >
      {props.children}
      {dragging && (
        <div ref={drop} className={realStyle('drop', dropClassname)}>
          {dropComponent}
        </div>
      )}
    </div>
