  ...
  const columns: ColumnType<RoleListItem>[] = [
    {
      ...
      render(name: string, item: RoleListItem) {
        return (
          <>
            {name}
            {item.roleType === 'SYSTEM' || item.roleType === 'ADMIN' ? <span className={style.systemRoleTag}>系统角色</span> : null}
          </>
        );
      },
    },
    ...
    },
    {
      ...
      render(_, item) {
        return (
          <>
            <a onClick={() => navigateTo('/#rbac?page=roleDetail&id=' + item.roleId)}>{(item as any).roleType === 'ORG' ? '编辑' : '查看'}</a>
  ...
  return (
    <div className={style.pageWrapper}>
      ...
      <div className={style.roleListWrapper}>
        ...
        <RoleMessageTip>若企业成员无任何角色，将无法操作相关功能模块。</RoleMessageTip>
        <Table
          rowKey="roleId"
