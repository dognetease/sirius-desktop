...
import { PageInfo } from '../../../mySite/components/pageInfo';
import { CreateSite } from '../../../mySite/components/CreateSite';
import { ReName } from '../../../mySite/components/ReName';
import { ChooseTemplate } from '../../../components/ChooseTemplate';
import { DataCard } from '../../../mySite/components/DataCard';
import { SiteCardHeader } from '../../../mySite/components/SiteCardHeader';
import { CreateCustomSite } from '../../../mySite/components/CreateCustomSite';
import { BindAnalysis } from '../../../mySite/components/BindAnalysis';
import { SITE_SWITCH_OPTIONS, DETAIL_IMG_URL, STATUS_ENUM, THEME, DOMAIN_STATUS } from '../../../mySite/constants';
...
import AICreateSiteCardBar from '../../../mySite/components/AICreateSiteCardBar';
import AICreateSiteModal from '../../../mySite/components/AICreateSiteModal';
import useIndustryList from '@web-site/hooks/useIndustryList';
import useSiteThemeList from '@web-site/hooks/useSiteThemeList';
import SiteServiceModal from '../../../mySite/components/SiteServiceModal';
import HttpsModal from '../../../mySite/components/HttpsModal';
...
import { TongyongWangye } from '@sirius/icons';
...
export const Site = () => {
  ...
  const [siteData, setSiteData] = useState<SiteDataItem[]>([]);
  const [siteItem, setSiteItem] = useState<React.ReactElement>();
  const [curItem, setCurItem] = useState<SiteDataItem>();
  ...
  // 获取官网卡片数据
  const getMySiteData = async (id = curItem?.siteId) => {
    // 存在上线官网
  ...
  const changeSite = async (id: string) => {
    const site = siteData.find(i => i.siteId === id);
    if (site) {
      const data = await getMySiteLatestData();
      const { siteId, isIndex } = site;
      const cardItemData = getStatisticalData(data[siteId], isIndex);
      const siteChildren = getSiteChild(site, cardItemData);
      setSiteItem(siteChildren);
      setCurItem({ ...site });
    }
  };
  ...
  // 获取官网和详情页的卡片列表
  const getSiteChildren = async (siteList: SiteDataItem[], id?: string): Promise<void> => {
    const data = await getMySiteLatestData();
    ...
    if (!siteList.length) {
      ...
      if (cardItemData.browseCount > 0 || cardItemData.submitCount > 0) {
        item = getSiteChild(
          {
            host: '',
            isIndex: false,
        ...
        const { siteId, isIndex } = defaultItem;
        const cardItemData = getStatisticalData(data[siteId], isIndex);
        item = getSiteChild(defaultItem, cardItemData);
      }
      setSiteItem(item);
      return;
    }

    const target = id ? siteList.find(site => site.siteId === id) : undefined;
    if (target) {
      const { siteId, isIndex } = target;
      const cardItemData = getStatisticalData(data[siteId], isIndex);
      const siteChildren = getSiteChild(target, cardItemData);
      setSiteItem(siteChildren);
      setCurItem({ ...target });
    } else {
      const { siteId, isIndex } = siteList[0];
      const cardItemData = getStatisticalData(data[siteId], isIndex);
      const siteChildren = getSiteChild(siteList[0], cardItemData);
      setSiteItem(siteChildren);
      setCurItem({ ...siteList[0] });
    }
  };
  ...
  const InSingleOption = (props: any) => {
      ...
      <Card
        ...
        ops={
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <Button onClick={openNewSite} className={styles.addSite} btnType="primary">
              <TongyongWangye wrapClassName="wmzz" size={16} />
              新建站点
            </Button>
            {siteData.length > 1 ? (
              <EnhanceSelect
                className={styles.siteSelect}
                style={{ marginLeft: '12px', maxWidth: '200px' }}
                value={curItem?.siteId}
                onChange={changeSite}
                optionLabelProp="label"
                dropdownStyle={{ width: '326px !important' }}
                dropdownMatchSelectWidth={326}
              >
                {siteData.map(item => {
                  const { siteName, indexUrl, siteId, data, status } = item;
                  return (
                    <InSingleOption className={styles.siteOption} key={siteId} name={siteName} label={siteName} value={siteId}>
                      <div className={styles.siteOptionContainer}>
                        <img src={data?.thumbnail} />
                        <div className={styles.siteOptionContent}>
                          <div className={styles.siteOptionTitle}>
                            <div>{siteName}</div>
                            <span className={status === STATUS_ENUM.ONLINE ? styles.onlineTag : styles.offlineTag}>
                              {status === STATUS_ENUM.ONLINE ? '上线中' : '未上线'}
                            </span>
                          </div>
                          <div className={styles.siteOptionHost}>{indexUrl}</div>
                        </div>
                        {curItem?.siteId === siteId ? <Selected /> : <Unselected />}
                      </div>
                    </InSingleOption>
                  );
                })}
