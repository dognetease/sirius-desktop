...
class CaptureEditor extends Event {
  ...
  async initImg(imageSrc) {
    ...
    electronLib.appManage.setOpacityShowCurrentWindow();
  ...
  async init() {
    let currentScreen = await getCurrentScreen();
    ...
    // ctx.drawImage(img, 0, 0);
    // this.bgCtx = ctx;
    await this.initImg(this.imageSrc);
  ...
  getImageUrl() {
    ...
    if (w && h) {
      ...
      ctx2.drawImage(canvas, 0, 0, w * scaleFactor, h * scaleFactor, 0, 0, w, h);
