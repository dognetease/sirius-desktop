...
import { safeDecodeURIComponent, getOfficialWebsiteHost } from '@web-common/utils/utils';
...
const loginType = 'mobile';

// eslint-disable-next-line max-statements
const MobileLogin: React.FC<LoginProps> = ({ type, initAccount, originLoginKey: loginInfoKey }) => {
  ...
  const loginSucc = () => {
    ...
    if (!type || type === 'common') {
      if (process.env.BUILD_ISEDM) {
        // 是否包含需要重定向地址
        if (/(?:^|\?|&)redirectUrl=([^&#]+)/.test(window.location.search)) {
          const url = safeDecodeURIComponent(RegExp.$1);
          console.log('redirectUrl', url);
          window.location.href = url;
        } else {
          window.location.replace(getOfficialWebsiteHost());
        }
        return;
      }
  ...
  const handleLoginReturn = (value: LoginModel) => {
    ...
    setAutoLogin(false);
    if (value.pass) {
      doLoginPageClickDataTrack(pageSource, 'true', undefined, loginType);
      loginSucc();
      ...
      needChangePwd();
      doLoginPageClickDataTrack(pageSource, 'false', getIn18Text('XUGENGHUANMIMA11'), loginType);
    }
  };
  ...
  const onLogin = () => {
    if (vaildateLoginForm(true)) {
      doLoginPageClickDataTrack(pageSource, 'false', getIn18Text('DENGLUDIANJI'), loginType);
      setLoading(true);
  ...
  const loginFormDiv = (
    <div className={classes.form}>
      ...
      <Button className={classnames({ [classes.submitBtn]: true, [classes.disabled]: !vaildateLoginForm() })} type="primary" onClick={onLogin} block loading={loading}>
        {getIn18Text('DENGLU')}
      </Button>
          ...
          <div className={classes.iconWrapper}>
            <div
              className={classnames(classes.icon, {
                [classes.iconEdm]: process.env.BUILD_ISEDM,
              })}
            />
            <div className={classes.text}>{getIn18Text('WANGYILINGXIBAN')}</div>
          </div>
          <header className={classes.title}>{getIn18Text('DENGLULINGXIBAN')}</header>
