...
import { apiHolder, apis, conf, DataStoreApi, DataTrackerApi, EventApi, inWindow, MailApi, NetStorageApi, SystemApi, SystemEvent, util, UpgradeAppApi } from 'api';
  ...
  return (
    <Tooltip key={name} title={realTitle} visible={showTooltip} overlayClassName={style.sideBarTipWrap} placement="right">
      <div
        ...
        onClick={handleClick}
        data-test-id={`side_bar_item_${name}_btn`}
...
const SideTabBar: React.FC<{
  ...
  const renderTabItem = useCallback(
    (params: { tab: SiriusPageProps; _unreadCount: { [key in PageName]?: number }; _activeKey: string; _hoverKey: string | undefined; showTooltip: boolean }) => {
      const { tab, _unreadCount, _activeKey, _hoverKey, showTooltip } = params;
    ...
    <DndProvider backend={HTML5Backend}>
      <div className={style.sideBarTicky}>
        <div className={classnames(style.sideBar, { [style.sideBarHeight]: inElectron() })} style={sideBarStyle}>
          ...
          {(systemApi.isElectron() || systemApi.inEdm()) && <Avatar notifyBlocking={blockFunc} activeKey={activeKey} />}
          {/* 从旧版进入，欢迎页modal */}
          {isWebmail && <WelcomeModal />}
          {shouldUpdateApp && <UpdateApp />}
          {tabsWithOrder.map(tab =>
            renderTabItem({
              tab,
              _unreadCount: unreadCount,
              _activeKey: activeKey,
              _hoverKey: hoverKey,
              showTooltip: tab.name === tooltip,
            })
          )}
