...
import { TongyongZhankaiYou, TongyongZhankaiZuo } from '@sirius/icons';
import { api } from 'api';
...
export const ExpandableSideContent: React.FC<SideContentLayoutProps & { isFold: boolean }> = props => {
  ...
  if (isFold) {
    return (
      <div className={style.expandWrapper}>
        <div className={classNames([style.expandBtn])} onClick={() => dispatch(ExpandMenuActions.setIsFold(false))}>
          <TongyongZhankaiYou />
        </div>
      </div>
    );
  ...
  return (
    <SideContentLayout
      className={style.foldWrapper}
      // eslint-disable-next-line react/jsx-props-no-spreading
      {...rest}
    >
      {/* <div className={style.foldWrapper}> */}
      <div className={classNames([style.foldBtn])} onClick={() => dispatch(ExpandMenuActions.setIsFold(true))}>
        <TongyongZhankaiZuo />
      </div>
      {/* </div> */}
      {children}
