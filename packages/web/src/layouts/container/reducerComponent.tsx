import { getIn18Text, syncRes, SystemEvent, EntityOrg, api, ContactPersonalMarkNotifyEventData, CustomerMapChangeEvent } from 'api';
...
import { doGetContactListAsync, refreshContactData, refreshContactDataByEmails } from '@web-common/state/reducer/contactReducer';
...
const ReducerComponent: React.FC = () => {
  ...
  const updateCallback = (e: SystemEvent) => {
    const { contact_personal: personalIdMap, contact_enterprise: enterpriseIdMap, isAll = false, emails } = e.eventData as syncRes;
    const { _account } = e;
    ...
    if (contactIdList.length && (personalIdMap || enterpriseIdMap)) {
      ...
      if (deleteList.length) {
        // TODO删除数据和新增数据合并成一个修改redux方式，而且应该重新获取email角色
    ...
    // 如果personal返回了email列表 更新redux email->id的映射关系
    if (emails && emails?.length && _account) {
      const emailMap = new Map(emails.map(email => [email, email]));
      refreshContactDataByEmails({ [_account]: emails }, emailMap);
    }
  };
