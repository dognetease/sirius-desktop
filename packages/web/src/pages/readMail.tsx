...
const ReadMailPage: React.FC<PageProps> = props => {
  ...
  const isPrivilegeSearch = getParameterByName('isPrivilege', location.search) || '';
  // 营销邮件回复邮箱非本账号邮箱
  const edmReplyConfig = useRef<{ edmEmailId: string; operateId?: string; bounceId?: string; isBounced?: boolean; isPrivilege?: boolean } | null>(
    ...
    edmEmailIdSearch && operateIdSearch
      ? {
          ...
          isPrivilege: isPrivilegeSearch === 'true',
        }
  ...
  useEffect(() => {
    ...
    const id = eventApi.registerSysEventObserver('initPage', {
      func: ev => {
        ...
        const isPrivilege = typeof ev?.eventData === 'string' ? '' : ev?.eventData?.isPrivilege;
        if (isTpMail) {
      ...
      func: ev => {
        try {
          if (ev) {
            ...
            if (mailAccount.current === agentEmail) {
              // 关闭当前窗口
              setTimeout(() => {
  ...
  return (
    <SiriusLayout.ContainerLayout isLogin={false} pages="readMail">
      {/* 客户端，点击页签触发的，才展示按钮 */}
