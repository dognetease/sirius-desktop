...
const TitleBar: React.FC<titleBar> = props => {
  const { isLogin, show = true, showClose = true, showMin = true, showMax = true, isLockApp = false, activeKey = '', pages } = props;
  ...
  // todo 区分外贸
  const mailBoxClass = isLockApp
    ? ''
    : window?.location?.hash?.includes('mailbox') || window?.location?.hash?.includes('enterpriseSetting?page=salesPitch')
    ? ' is-mail-box'
    : '';
        ...
        <div className={`btn-group-win ${isImageWindow() && 'image-window-btn'}`} style={style}>
          {showMin && (
            <div className="browser-btn min-btn" onClick={min}>
              <i className="dark-invert min"> </i>
            </div>
          )}
          {showMax && (
            <div className="browser-btn max-btn" onClick={max}>
              <i className={`dark-invert ${isMax ? 'un-max' : 'max'}`} />
            </div>
          )}
          {showClose && (
            <div className="browser-btn close-btn" onClick={close}>
              <i className="dark-invert close" />
            </div>
          )}
        </div>
      </div>
  ...
  const exthemeClass =
    ['setting', 'schedule', 'contact', 'message', 'disk'].includes(activeKey) || ['readMail', 'scheduleOpPage', 'writeMail'].includes(pages) ? '' : 'extheme';
   ...
   */
  return (
    <div id="global-window-operate-bar" className={`${exthemeClass} title-bar-wrap ${themeClass} ${mainClass}`}>
      {renderTitleBar()}
    </div>
  );
};
