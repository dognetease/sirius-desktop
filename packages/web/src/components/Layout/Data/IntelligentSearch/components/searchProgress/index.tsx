...
export const SearchProgress: React.FC<SearchProgressProps> = props => {
  const { visible, total, minimize, taskStatus, isManualStop, onMinimizeChange, onStop, onFinish } = props;
    ...
    switch (true) {
      ...
      case percent >= 30 && percent <= 100:
        return (
          <>
            已搜到
            <span className={style.highlight}>{` ${Number(total).toLocaleString()} 条 `}</span>
            数据...
          </>
        );
      default:
  ...
  const getMaximizeStepByPercent = (percent: number) => {
    switch (true) {
      case percent < 15:
        return (
          <>
            <div className={style.name}>{getTransText('StartEngineSearching') || ''}</div>
      ...
      case percent < 30:
        return (
          <>
            <div className={style.name}>{getTransText('ExpandSearchKeywords') || ''}</div>
            <div className={style.icon}>
              <AiSearchSearching className={style.spining} />
            </div>
          </>
      ...
      case percent < 90:
        return (
          <>
            <div className={style.name}>{getTransText('MainstreamPlatformDataAcquisition') || ''}</div>
            <div className={style.icon}>
              <AiSearchSearching className={style.spining} />
            </div>
          </>
      ...
      case percent < 100:
        return (
          <>
            <div className={style.name}>{getTransText('ConsolidateSearchResults') || ''}</div>
            <div className={style.icon}>
              <AiSearchSearching className={style.spining} />
            </div>
          </>
    ...
    content = (
      <div className={style.content}>
        ...
        <div className={style.steps}>
          ...
          <div className={style.step}>
            <div className={style.name}>{getTransText('SmartExtractionOfWhatsAppNumbers') || ''}</div>
            <div className={style.dashed} />
            <div className={style.icon}>
              {percent >= 0 && percent < 30 && <AiSearchWaitting />}
              {percent >= 30 && percent < 90 && <AiSearchSearching className={style.spining} />}
          ...
          <div className={style.step}>
            <div className={style.name}>{getTransText('VerifyNumberAccuracy') || ''}</div>
            <div className={style.dashed} />
            <div className={style.icon}>
              {percent >= 0 && percent < 30 && <AiSearchWaitting />}
              {percent >= 30 && percent < 90 && <AiSearchSearching className={style.spining} />}
          ...
          <div className={style.step}>
            ...
            <div className={style.dashed} />
            <div className={style.icon}>
              {percent >= 0 && percent < 90 && <AiSearchWaitting />}
              {percent >= 90 && percent < 100 && <AiSearchSearching className={style.spining} />}
              {percent === 100 && <AiSearchFinished />}
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      <Modal width={!minimize ? 420 : 442} className={style.searchProgress} closable={false} visible={visible} footer={null} getContainer={false}>
        {content}
      </Modal>
    </div>
  );
