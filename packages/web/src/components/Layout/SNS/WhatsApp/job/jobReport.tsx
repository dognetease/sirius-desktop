...
import {
  ...
  WhatsAppJobReceiverSendState,
  getWhatsAppJobReceiverSendStateName,
  WhatsAppJobReceiverSendStateColor,
...
import { getPreviewTextFromTemplate, fillTemplateWithTemplateParams } from '@/components/Layout/SNS/WhatsApp/utils';
import MessageContext, { messageContext } from '../context/messageContext';
...
const WhatsAppJobReceiverSendStateName = getWhatsAppJobReceiverSendStateName();
...
const SEND_STATE_OPTIONS = Object.values(WhatsAppJobReceiverSendState)
  .filter(value => typeof value === 'number')
  .map(value => ({
    label: WhatsAppJobReceiverSendStateName[value as WhatsAppJobReceiverSendState],
    value,
  }));
...
const JobReport: React.FC<JobReportProps> = props => {
  ...
  const handleJobDetailFetch = (jobId: string) => {
    whatsAppApi.getJobDetail({ jobId: qs.jobId }).then(nextJobDetail => {
      setJobDetail(nextJobDetail);
      if (nextJobDetail.templateId) {
        whatsAppApi.getTemplateDetail({ id: nextJobDetail.templateId }).then(nextTemplate => {
          const structure = JSON.parse(nextTemplate.structure as unknown as string);
          setTemplate({ ...nextTemplate, structure });
        });
      }
    });
  };
  ...
  useEffect(() => {
    if (params) {
      ...
      whatsAppApi.getJobReportStat(restParams).then(data => {
        if (didCancel) return;
        setStatData(data);
      });
      return () => {
  ...
  const columns: ColumnsType<WhatsAppJobReportReceiver> = [
    ...
    },
    {
      ...
      width: 90,
      dataIndex: 'sendState',
      render: (value: WhatsAppJobReceiverSendState) => <span style={{ color: WhatsAppJobReceiverSendStateColor[value] }}>{WhatsAppJobReceiverSendStateName[value]}</span>,
    },
    {
    ...
    },
    {
      ...
      render: (_: any, row: WhatsAppJobReportReceiver) => (
        <PrivilegeCheck accessLabel="OP" resourceLabel="WHATSAPP">
          <a onClick={() => navigate(`#edm?page=whatsAppMessage&chatId=${row.receiverPhone}`)}>{getIn18Text('LIAOTIANJILU')}</a>
        </PrivilegeCheck>
      ),
    },
  ];
  const handleTemplatePreview = (template: WhatsAppTemplate) => {
    if (!jobDetail) return;
  ...
  const handleCopy = (jobDetail: WhatsAppJobDetail) => {
    ...
    <PermissionCheckPage resourceLabel="WHATSAPP" accessLabel="VIEW" menu="WHATSAPP_SEND_TASK">
      <MessageContext.Provider value={messageContext}>
        <div className={classnames('edm', edmStyle.container, style.jobReport)}>
          ...
          <Quota className={style.bannerQuota} type="banner" ref={quotaRef} />
