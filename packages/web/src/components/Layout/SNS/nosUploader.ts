...
import { apis, apiHolder, ApiRequestConfig, WhatsAppApi, SnsNosUploadInfo } from 'api';
...
export class NosUploader extends EventEmitter {
  ...
  startUpload() {
    ...
    getUploadToken(option)
      .then(data => {
        this.uploadInfo = data as unknown as SnsNosUploadInfo;
      ...
      .then(ret => {
        if (ret.status === 200) {
          if (isComplete) {
            ...
            (this.uploadInfo as SnsNosUploadInfo).context = data.context;
