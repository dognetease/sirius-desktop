...
import CrmPicker from './crmPicker';
...
interface ReceiverProps {
  ...
  onExtracted: (extractType: ReceiverTab, extractResult: WhatsAppFileExtractResult) => void;
  onFilter?: (filterWhatsApp: string[]) => void;
  onFilterStatusChange: (status: FilterStatus) => void;
}
export type ReceiverTab = 'file' | 'text' | 'crm';
const Receiver: React.FC<ReceiverProps> = props => {
  ...
  const [activeTab, setActiveTab] = useState<ReceiverTab>('file');
  ...
  const extractCallback = (extractType: ReceiverTab, extractResult: WhatsAppFileExtractResult) => {
  ...
  return (
    <div className={classnames('edm', style.receiver, className)}>
      <div className={style.picker}>
        <Tabs className="custom-ink-bar" activeKey={activeTab} onChange={nextTab => setActiveTab(nextTab as ReceiverTab)}>
          ...
          <TabPane className={style.pickerContent} key="crm" tab={getIn18Text('KEHUGUANLIXUANZE')}>
            <CrmPicker extraction={null} onExtracted={extractResult => extractCallback('crm', extractResult)} />
          </TabPane>
        </Tabs>
      </div>
