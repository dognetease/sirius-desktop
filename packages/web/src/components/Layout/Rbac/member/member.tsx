...
import { getContact, getSearchContact } from '@web-common/utils/contact_util';
    ...
    >
      <SideContentLayout minWidth={192} defaultWidth={192} borderRight>
        <div style={{ padding: '8px 8px 0' }}>
          <Input
            ...
            max={100}
            placeholder={getIn18Text('SOUSUOQIYECHENGYUAN')}
            onChange={handleSearch}
            prefix={<div className={`dark-invert ${styles.searchIcon}`} />}
            allowClear
          />
        </div>
      ...
      {displayContactList.length === 0 && (
        <div className={styles.emptyContainer}>
          ...
          <div>{getIn18Text('ZANWULIANXIREN')}</div>
        </div>
      )}
      {displayContactList.length !== 0 && (
        <>
...
const RoleInfoForContact: React.FC<{
  orgId: string;
  accId: string;
}> = props => {
  const appDispatch = useAppDispatch();
  ...
  const handleCheck = (id: string, e: CheckboxChangeEvent) => {
    roleApi
      ...
      .then(() => {
        fetchRole();
        toast.success({ content: (e.target.checked ? getIn18Text('TIANJIA') : getIn18Text('SHANCHU')) + getIn18Text('JIAOSECHENGGONG') });
      });
  };
  ...
  return (
    <div className={styles.roleInfoRow}>
      <div className={styles.roleInfoLabel}>{getIn18Text('JIAOSE')}</div>
      <div className={styles.roleInfoContent}>
        {roles.length > 0 ? (
          ...
          <div>{getIn18Text('QINGTIANJIAJIAOSE')}</div>
        )}
      </div>
