...
export const TreeSelect = (props: ITreeSelectProps) => {
  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, borderLeft: '1px solid rgba(38, 42, 51, .16)', padding: '12px 12px 0' }}>
        {data.length > 0 && <ContactList selectList={data} showTitle={false} showFooter={false} onDelete={handleDelete} />}
        {data.length === 0 && (
          <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <div style={{ color: 'rgba(38, 42, 51, 0.5)', fontSize: 12 }}>{getIn18Text('ZANWEITIANJIARENHECHENGYUAN')}</div>
          </div>
        )}
      </div>
    </div>
