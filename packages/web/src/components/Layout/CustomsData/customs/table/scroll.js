class AutoScroll {
  ...
  createWidthScroll(w1, w2, h1, h2) {
    ...
    // scrollBox.style.top = this.offsetVal(this.obj).offsetT + this.obj.offsetHeight + 10 + "px";
    // console.log('xxxxx-heigit', this.offsetVal(this.obj), this.obj.clientHeight, window.innerHeight);

    scrollBox.innerHTML = `<div class='customs-scroll-tool' style='width: ${(w1 / w2) * w1}px;' </div>`;
    // if (this.obj.clientHeight + this.offsetVal(this.obj).offsetT < window.innerHeight) {
    // 	scrollBox.style.bottom = `${this.obj.clientHeight + this.offsetVal(this.obj).offsetT}px`;
    // }
    const isScrollBox = this.obj.querySelector('.customs-scroll-box');
    // console.log('xxxxisScrollBox', this.obj, isScrollBox)
    if (isScrollBox) {
      isScrollBox.remove();
      // this.obj.removeChild(scrollBox);
    }
    this.obj.appendChild(scrollBox);
  ...
  drag(x1, y1) {
    ...
    document.onmousemove = event => {
      ...
      //   this.scrollContent.style.marginLeft = -((topValue / this.obj.offsetWidth) * this.obj.scrollWidth) + "px";
