...
import { SidebarClueInfo } from './clueInfoNew';
...
const SWITCH_ICON_TIP_KEY = 'SWITCH_CLUE_ICON_TIP';

export const ClueSidebar = ({ email, name, onClickHelp, _account = getMainAccount(), setLoading, noBorder }: ClueSidebarProps) => {
  ...
  // 是否是无权限的同事客户
  const isNoAuth = useMemo(() => emailRole && emailRole === 'colleagueClueNoAuth', [emailRole]);
  // 当前选择的客户id
  ...
  const topTitle = useMemo(() => {
    if (emailRole) {
      const titltObj: Partial<Record<EmailRoles, string>> = {
        myClue: getIn18Text('XIANSUOXIANGQING'),
        colleagueClue: getIn18Text(['TONGSHIXIANSUO', 'XIANGQING']),
        colleagueClueNoAuth: getIn18Text(['TONGSHIXIANSUO', 'XIANGQING']),
        openSeaClue: getIn18Text(['GONGHAIXIANSUO', 'XIANGQING']),
      };
      return titltObj[emailRole as EmailRoles] || getIn18Text('XIANSUOXIANGQING');
    } else {
      return getIn18Text('XIANSUOXIANGQING');
    }
  }, [emailRole]);

  // 判断是否是公海客户
  const isOpenSea = useMemo(() => ['openSeaClue'].includes(emailRole), [emailRole]);
  // 详情，监听客户详情的变化
  ...
  const handleUniDrawer = (type: 'clue' | 'contact', contactId?: string) => {
    if (detail?.company_id) {
      ...
      if (type === 'clue') {
        showUniDrawer({
          ...
          moduleProps: {
            visible: true,
            leadsId: Number(detail.company_id),
        ...
        showUniDrawer({
          ...
          moduleProps: {
            ...
            onSuccess: () => {
              refreshDetail();
              customerInfoRef.current?.setActiveTab('3'); // 展示客户基本信息
            },
    ...
    >
      <div className={style.columnFlexContainer}>
          ...
          <div>
            ...
            {itemList.length > 1 && (
              <Dropdown overlay={overlay} trigger={['click', 'click']} overlayClassName={style.switchCustomerOverlay}>
                <Tooltip placement="bottom" title={getIn18Text('ZAIZHELIQIEHUAN、CKQTXS')} visible={switchIconTip}>
        ...
        <SidebarClueInfo
          ...
          originName={name}
          actionKeys={isOpenSea ? ['getClue'] : ['edit', 'newSchedule', 'newFollow', 'returnOpenSea', 'toCustomer']}
        />
      </div>
    </div>
  );
