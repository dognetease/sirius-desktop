...
const ForwardPicker: React.FC<ForwardPickerProps> = props => {
  ...
  sessionList.forEach((item: any) => {
    ...
    item.userInfo = { account, avatar, name, color };
  });
    ...
    >
      <div className={style.picker}>
        <div className={style.searchFilter}>
          <Input placeholder={getIn18Text('SOUSUOLIANXIREN')} value={searchKey} onChange={event => setSearchKey(event.target.value)} />
        </div>
        <div className={style.recentContact}>{getIn18Text('ZUIJINHUIHUA')}</div>
