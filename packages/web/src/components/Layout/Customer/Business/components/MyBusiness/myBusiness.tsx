...
import { businessDataTracker, BatchOperationType } from '../../../tracker/businessDataTracker';
...
import { apiHolder, apis, CustomerApi, opportunityListReq, opportunityCheckExportReq as exportType, urlStore } from 'api';
...
const MyClue: React.FC<ComsProps> = ({ message, closeMessage, tableEvent, marketingEvent }) => {
  ...
  const { loading, isSHowSkeleton, hasClue, tableList, total, requestTableData, current } = useTableDataLoader(clientApi, 'opportunityList', searchParam);
  ...
  useJumpPage(examineBusiness, 'business');
  /*
   * 关闭编辑弹框
   */
  ...
  return (
    <div className={style.myClueWrap}>
      <Search className={style.tabsTop} handerSearch={handerSearch} isShowManager={false} />
      <Skeleton active loading={isSHowSkeleton} paragraph={{ rows: 4 }}>
        ...
        {hasClue && (
          <div ref={tableRef} className={style.tableWrap}>
            <Table
              tableWidthKey="wm-customer-mybusiness"
      ...
      >
        ...
        <PrivilegeCheck accessLabel="OP" resourceLabel="COMMERCIAL">
          ...
          {/* <Button style={{ marginLeft: 12 }} onClick={ changeClueStatus }>修改商机状态</Button> */}
        </PrivilegeCheck>
