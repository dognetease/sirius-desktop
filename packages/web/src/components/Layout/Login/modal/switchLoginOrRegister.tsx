...
const SwitchLoginOrRegister: React.FC<SwitchLoginOrRegisterProp> = props => {
  ...
  const switchRegister = async () => {
    ...
    try {
      ...
      if (success) {
        if (isRegister) {
          const msg = getIn18Text('MEIGESHOUJIHAOJINZHI');
          Message.error(msg);
    ...
    >
      <div className={styles.wrap}>
        <div className={styles.title}>
          ...
          <div className={styles.titleTxt2}>{getIn18Text('NINDESHOUJIHAOHAIWEI')}</div>
        </div>
        <div className={styles.content}>
          <div className={styles.item} onClick={switchLogin}>
            <div className={styles.loginIcon} />
            <div className={styles.label}>{getIn18Text('BANGDINGYIYOUDEWANGYI')}</div>
            <div className={styles.arrowRight} />
          </div>
          <div className={styles.item} onClick={switchRegister}>
            <div className={styles.registerIcon} />
            <div className={styles.label}>{getIn18Text('ZHUCENINDEWANGYIWAI')}</div>
            <div className={styles.arrowRight} />
          </div>
        </div>
      </div>
