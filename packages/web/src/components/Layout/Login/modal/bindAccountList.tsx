...
import MailBindModal from '@/components/Layout/Login/modal/bindAccount';
...
const BindAccountListModal = () => {
  ...
  const switchAccount = ({ accountName, domain, token, enabledMobileLogin, status, area }: MobileAccountInfo) => {
    ...
    if (String(status) !== '0') {
      SiriusModal.error({
        title: getIn18Text('ZHANGHAOWUFADENG'),
        hideCancel: true,
        content: getIn18Text('DANGQIANNINDEZHANGHAOWU'),
        onOk: () => {
          setMobileBindAccountListInfo({
            visibleList: !(isFromLogin && isLastAccount),
          });
  ...
  const title = getIn18Text('XUANZENINYAODENGLUDE');
  ...
  return (
    <>
      ...
      >
        ...
        <MailBindModal
          from="login"
          title={getIn18Text('YANZHENGNINDEWANGYIWAI')}
