...
const { renderToString, renderToStaticMarkup, pipeToNodeWritable } = require(`react-dom/server`);
...
export default async function staticPage({ pagePath, pageData, staticQueryContext, styles, scripts, reversedStyles, reversedScripts, inlinePageData = false }) {
  // for this to work we need this function to be sync or at least ensure there is single execution of it at a time
  global.unsafeBuiltinUsage = [];

  try {
    let bodyHtml = ``;
    let headComponents = [<meta name="generator" content={`Gatsby ${gatsbyVersion}`} key={`generator-${gatsbyVersion}`} />];
    let htmlAttributes = {};
    let bodyAttributes = {};
    ...
    class RouteHandler extends React.Component {
      render() {
        ...
        const pageElement = null;

        const wrappedPage = apiRunner(`wrapPageElement`, { element: pageElement, props }, pageElement, ({ result }) => {
    ...
    const bodyComponent = (
      <StaticQueryContext.Provider value={staticQueryContext}>
        {apiRunner(`wrapRootElement`, { element: routerElement, pathname: pagePath }, routerElement, ({ result }) => {
    ...
    if (!bodyHtml) {
      try {
        ...
        if (pipeToNodeWritable) {
          ...
          const { startWriting } = pipeToNodeWritable(bodyComponent, writableStream, {
            onCompleteAll() {
              startWriting();
            },
            onError() {},
    ...
    reversedScripts.forEach(script => {
      ...
      headComponents.push(<link as="script" rel={script.rel} key={script.name} href={`${__PATH_PREFIX__}/${script.name}`} />);
    });

    if (pageData && !inlinePageData) {
      headComponents.push(<link as="fetch" rel="preload" key={pageDataUrl} href={pageDataUrl} crossOrigin="anonymous" />);
    }
    staticQueryUrls.forEach(staticQueryUrl => headComponents.push(<link as="fetch" rel="preload" key={staticQueryUrl} href={staticQueryUrl} crossOrigin="anonymous" />));

    const appDataUrl = getAppDataUrl();
    if (appDataUrl) {
      headComponents.push(<link as="fetch" rel="preload" key={appDataUrl} href={appDataUrl} crossOrigin="anonymous" />);
    }
    ...
    reversedStyles.forEach(style => {
      ...
      if (style.rel === `prefetch`) {
        headComponents.push(<link as="style" rel={style.rel} key={style.name} href={`${__PATH_PREFIX__}/${style.name}`} />);
      } else {
