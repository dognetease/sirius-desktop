/**
 * ! 注意：v2版本只有内容库列表在使用，普通写信模板列表还在使用旧版列表
 */
import React, { FC, useState, useCallback, useEffect, useRef } from 'react';
import { Select, Tabs, message, Modal, Skeleton, Popover } from 'antd';
// import SearchOutlined from '@ant-design/icons/SearchOutlined';
import classnames from 'classnames/bind';
import { navigate } from '@reach/router';

import { apiHolder, apis, MailTemplateApi, TemplateConditionRes, GetTemplateListReq, DataStoreApi, DataTrackerApi, UpdateTimeProps, EdmSendBoxApi } from 'api';
...
import { MailTemplateListV2, ListType } from './MailTemplateListV2/MailTemplateListV2';
...
import { VideoModal, PlayState } from '@web-common/components/UI/Video';
...
import { TemplateListV2 } from './TemplateListV2';
import { SearchPage } from './MailTemplateListV2/SearchPage';
...
type Page = 'home' | 'taskList' | 'template';
...
export const MailTemplate: FC<{
  ...
  const [modalType, setModalType] = useState<'task' | 'template'>('task');
  ...
  // 埋点
  useEffect(() => {
    edmDataTracker.templatePageEnter();
  }, []);
  ...
  const renderSearch = () => (
    <>
      <div className={styles.topOp}>
        <div className={styles.headTitle}>内容库</div>
        <div className={styles.rightFilter}>
          ...
          <Input
            ...
            onPressEnter={evt => {
              edmDataTracker.templatePageOp('search', '');
          ...
          <Button
            btnType="minorLine"
            onClick={() => {
              edmDataTracker.templatePageOp('draft', '');
              navigate(`${routerWord}?page=drafts`);
            }}
          >
            查看草稿
          </Button>
          <CreateTemplateBtn goTaskList={goTaskList} goRecommendPage={goRecommendPage} goTemplateAdd={goTemplateAdd} addNewTemplate={addNewTemplate} />
        </div>
      </div>
    </>
  );
  ...
  const renderList = () => (
    <>
      {searchValue ? (
        ...
        <MailTemplateListV2
          showMore={(listTitle, listType, listId) => {
            setListType(listType);
            if (listType === 'task') {
              setPage('taskList');
            } else {
              setPage('template');
              setPageTitle(listTitle);
              setPageId(listId);
            }
          }}
  ...
  const renderModal = () => (
    <SiriusModal
      ...
      title={modalType === 'task' ? '最近任务' : '推荐模板'}
      width={900}
      footer={null}
    >
      {modalType === 'task' ? (
        ...
        <TemplateListV2
          ...
          goTemplateAdd={goTemplateAdd}
          useRecommendContent={content => addNewTemplate(content)}
          goTaskList={goTaskList}
          onUse={(id, content) => addNewTemplate(content)}
        />
      )}
    </SiriusModal>
  );
  ...
  return (
    <div className={styles.templateWrapper}>
      <div className={styles.templateWrapperContent}>
        {/* 头部 */}
        <div className={styles.templateHeader}>
          {renderSearch()}
          {/* {searchValue && renderTabs()} */}
          {/* {renderAggregation()} */}
        </div>
        {/* 列表区域 */}
        {renderList()}
