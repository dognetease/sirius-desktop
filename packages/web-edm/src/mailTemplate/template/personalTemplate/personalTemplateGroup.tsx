...
import classes from './personalTemplateGroup.module.scss';
import { apiHolder as api, apis, DataTrackerApi, MailTemplateApi, WaimaoRecommendTemplateTag } from 'api';
import PersonalTemplateMultiSelect from './personalTemplateMultiSelect';
...
const PersonalTemplateGroup: React.FC<Props> = ({ originTagIds, manageGroup, sendSelectedIds, manageVisible }) => {
  ...
  const groupNameRender = () => {
    if (selectedTagIds.length === 0) {
      return <span className={classes.groupPlaceholderTips}>请选择</span>;
    }
    const showName =
      tagList.find(item => {
...
export default PersonalTemplateGroup;
