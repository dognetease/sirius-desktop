...
import { apiHolder, apis, EdmSendBoxApi, SendBoxSender, MailConfApi, AccountApi, SenderListV2Resp, SenderRotateList } from 'api';
...
import { EmailSenderList } from '../../senderRotate/emailSenderList';
...
// 赠送域名屏蔽
export const PRESENT = 'ntesmail.com';
...
export interface SenderEmailProps {
  ...
  /**
   * 从第一次拿过来的senderEmails
   */
  initSenderEmails?: Array<string>;
  /**
   * 0：不一致，1：一致
   */
  senderType?: 0 | 1;
   ...
   * senderType 变化
   */
  senderTypeChange?: (value: 0 | 1) => void;
}
export const SecondSenderEmail: React.FC<SenderEmailProps> = prop => {
  // 登录用户主账号
  ...
  const { form, leftMargin = 0, name = 'senderEmail', onSenderEmailChange, isSecondSend = false, senderType = 0, senderTypeChange } = prop;
  ...
  // const { state: writeContext } = useContext(edmWriteContext)?.value;
  const initSenderEmails = useContext(edmWriteContext) ? useContext(edmWriteContext).value?.state?.senderEmails || [] : [];

  const [personalSenderList, setPersonalSenderList] = useState<SendBoxSender[]>([]);
  const [multiDomainList, setMultiDomainList] = useState<SenderRotateList['accounts']>([]);
  // 发件地址类型
  const [type, setType] = useState(senderType);
  const [curSenderEmail, setCurSenderEmail] = useState('');
  const [loading, setLoading] = useState(false);
  // 固定发件地址
  const [senderEmails, setSenderEmails] = useState<Array<string>>([]);
  ...
  const fetchSenderList = async () => {
    ...
    try {
      ...
      // resp.assignSenders?.map((i, index) => {
      //   i.unavailable = index % 2 === 0;
      // });

      setPersonalSenderList(list);
      // 只有明确有权限的情况下, 才展示这个
      hasMultiDomainPermission && setMultiDomainList(resp.assignSenders);
      setLoading(false);
    } catch (e) {
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    const senderEmail: string = form.getFieldValue(name);
    // const senderEmails: Array<string> = form.getFieldValue('senderEmails');
    // 下拉默认填当前主账号
    const email = systemApi.getCurrentUser()?.id || '';
    // const curEmail = senderEmails ?? (senderEmail ? [senderEmail] : email ? [email] : '');
    setSenderEmails([email]);
  }, [form]);
  ...
  // senderEmails 变了之后修改form中的值
  useEffect(() => {
    form.setFields([{ name: 'senderEmails', value: senderEmails }]);
  }, [senderEmails]);

  useEffect(() => {
    senderTypeChange && senderTypeChange(type);
  }, [type]);
      ...
      <>
        ...
        {available ? (
          selectItem && [selectItem.spf1Status, selectItem.dkimStatus, selectItem.dmarcStatus].includes(2) ? (
              ...
              <Popover
                ...
                title={
                  <div className={style.senderEmailPopoverContent}>
                    <p className={style.popoverTitle}>基础认证（基础认证完善有助于提升送达率）：</p>
                    ...
                    <Divider margin={12} />
                    <p className={style.popoverTitle}>配置方式（可在微信群中联系服务助手，获取人工服务）：</p>
            ...
            <div className={style.tip} style={{ marginLeft: `${leftMargin}px` }}>
              用不同域名的发件地址发营销邮件可提高送达率，在管理后台绑定新域名。
            </div>
          )
        ) : (
    ...
    if (isSecondSend) {
      return (
        <EnhanceSelect
          ...
          placeholder={available ? getIn18Text('ZHICHIGENGHUANFAJIANDE') : '无可用发件地址'}
          value={senderEmails[0]}
          onChange={value => {
            form.setFields([{ name: 'senderEmails', value: [value] }]);
            setSenderEmails([value]);
          }}
        >
          {[...personalSenderList!, ...multiDomainList!].map(item => (
            <InSingleOption disabled={!!item?.unavailable} key={item.email} value={item.email}>
              <>
                {item.email}
        ...
        >
          <SiriusRadio.Group
            ...
            onChange={e => {
              ...
              if (e.target.value === 1) {
                setSenderEmails(initSenderEmails);
              } else {
                const email = systemApi.getCurrentUser()?.id || '';
                setSenderEmails([email]);
              }
          ...
          >
            <SiriusRadio value={0}>固定发件地址</SiriusRadio>
            <SiriusRadio value={1}>同此任务首次发件一致</SiriusRadio>
          </SiriusRadio.Group>
        </div>
        {type === 0 ? renderSelectOption() : <EmailSenderList displayOnly preCheckList={initSenderEmails} />}
      </Form.Item>
      {renderSenderEmailTips()}
      {/* {selectItem ? (
        <div className={style.tip} style={{ marginLeft: `${leftMargin}px` }}>
