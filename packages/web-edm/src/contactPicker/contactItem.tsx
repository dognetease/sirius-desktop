...
const ContactItem: React.FC<ContactItemProps> = props => {
  ...
  return (
    <div
      ...
      onClick={event => !disabled && !!email && onClick && onClick(event)}
    >
      {checkable && <Checkbox checked={checked} disabled={disabled || !email} />}
      <AvatarTag className={style.avatar} user={{ name, email }} size={32} innerStyle={{ border: 'none' }} />
      <div className={style.content}>
        <div className={style.name}>
          {name || email || <span className={style.noContent}>{getIn18Text('(WUNEIRONG)')}</span>}
          {!email && <span className={style.noEmail}>{getIn18Text('WUYOUXIANGDEZHI')}</span>}
        </div>
