...
export interface GenerateConfig<OptionsType extends object[]> {
  ...
  components: {
    optionList: React.ForwardRefExoticComponent<
      React.PropsWithoutRef<Omit<OptionListProps<OptionsType>, 'options'> & { options: OptionsType }> & React.RefAttributes<RefOptionListProps>
    >;
  };
  ...
  | ((values: RawValueType[], options: FlattenOptionsType<OptionsType>) => OptionsType)
    ...
    | ((values: RawValueType[], options: FlattenOptionsType<OptionsType>, info?: { prevValueOptions?: OptionsType[] }) => OptionsType);
...
>(config: GenerateConfig<OptionsType>) {
  ...
  function Select<ValueType extends DefaultValueType>(props: SelectProps<OptionsType, ValueType>, ref: React.Ref<RefSelectProps>): React.ReactElement {
    const {
      prefixCls = defaultPrefixCls,
      className,
      id,
    ...
    const tokenWithEnter = useMemo(() => (tokenSeparators || []).some(tokenSeparator => ['\n', '\r\n'].includes(tokenSeparator)), [tokenSeparators]);
    ...
    // ============================== Ref ===============================
    const selectorDomRef = useRef<HTMLDivElement | null>(null);
    ...
    /** Unique raw values */
    const [mergedRawValue, mergedValueMap] = useMemo<[RawValueType[], Map<RawValueType, LabelValueType>]>(
      () =>
    ...
    const mergedFlattenOptions: FlattenOptionsType<OptionsType> = useMemo(() => flattenOptions(mergedOptions, props), [mergedOptions]);
    ...
    const displayOptions = useMemo<OptionsType>(() => {
      if (!mergedSearchValue || !mergedShowSearch) {
        // console.log('[selector] displayOptions mergedOptions.length', mergedOptions.length);
      ...
      // console.log('[selector] displayOptions filteredOptions',filteredOptions, mergedSearchValue, mergedOptions);
      // if (
      //   mode === 'tags' &&
    ...
    const displayFlattenOptions: FlattenOptionsType<OptionsType> = useMemo(() => flattenOptions(displayOptions, props), [displayOptions]);
    ...
    const triggerChange = (newRawValues: RawValueType[], type) => {
    ...
    const onInternalSelect = (newValue: RawValueType, { selected, source }: { selected: boolean; source: 'option' | 'selection' }) => {
      if (disabled) {
        return;
      }
      ...
      if (isMultiple) {
        ...
        if (selected) {
          if (newRawValue.has(newValue)) {
            // 如果有重复的不能什么都不做，业务需求
            // 所以此处提前结束set结构
            newRawValue = Array.from(newRawValue);
            // 保证最新的在最后面
            newRawValue.push(newValue);
          } else {
    ...
    const customizeInputElement: React.ReactElement = (mode === 'combobox' && getInputElement && getInputElement()) || null;
    ...
    // console.log('[selector] triggerOpen', triggerOpen);
    const onToggleOpen = (newOpen?: boolean) => {
      const nextOpen = newOpen !== undefined ? newOpen : !mergedOpen;

      if (innerOpen !== nextOpen && !disabled) {
        setInnerOpen(nextOpen);
        // console.log('[selector] onToggleOpen', nextOpen);
    ...
    const triggerSearch = (searchText: string, fromTyping: boolean, isCompositing: boolean) => {
      ...
      const patchLabels: string[] = isCompositing ? null : getSeparatedContent(searchText, tokenSeparators);
      ...
      if (mode === 'combobox') {
        ...
        if (mode !== 'tags') {
          patchRawValues = patchLabels
            .map(label => {
              const item = mergedFlattenOptions.find(({ data }) => data[mergedOptionLabelProp] === label);
              return item ? item.data.value : null;
            })
            .filter((val: RawValueType) => val !== null);
        }

        const newRawValues = Array.from(new Set<RawValueType>([...mergedRawValue, ...patchRawValues]));
    ...
    useEffect(() => {
      if (innerOpen && !!disabled) {
        setInnerOpen(false);
        // console.log('[selector] disabled', innerOpen, disabled);
      }
    }, [disabled]);
    ...
    const onInternalKeyDown: React.KeyboardEventHandler<HTMLDivElement> = (event, ...rest) => {
      ...
      if ([KeyCode.DELETE, KeyCode.BACKSPACE].includes(which) && !clearLock && isMultiple && !mergedSearchValue && mergedRawValue.length) {
        ...
        if (chosenList.length) {
          ...
          triggerChange(newValues, 'delete');
          ...
          // 选中之前的胶囊
          const offset = which === KeyCode.BACKSPACE ? 0 : 1;
          const index = selectorRef.current.getLastEnabledIndex() + offset;
    ...
    const onInternalMouseDown: React.MouseEventHandler<HTMLDivElement> = (event, ...restArgs) => {
      const { target } = event;
      const popupElement: HTMLDivElement = triggerRef.current && triggerRef.current.getPopupElement();
    ...
    if (!disabled && allowClear && (mergedRawValue.length || mergedSearchValue)) {
      clearNode = (
        <TransBtn className={`${prefixCls}-clear`} onMouseDown={onClearMouseDown} customizeIcon={clearIcon}>
          ×
        </TransBtn>
      );
    }
    ...
    // ============================= Arrow ==============================
    const mergedShowArrow = showArrow !== undefined ? showArrow : loading || (!isMultiple && mode !== 'combobox');
