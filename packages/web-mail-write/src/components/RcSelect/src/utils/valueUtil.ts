...
import type { OptionsType as SelectOptionsType, OptionData, OptionGroupData, FlattenOptionData } from '../interface';
import type { LabelValueType, FilterFunc, RawValueType, GetLabeledValue, DefaultValueType } from '../interface/generator';
...
export function findValueOption(values: RawValueType[], options: FlattenOptionData[], { prevValueOptions = [] }: { prevValueOptions?: OptionData[] } = {}): OptionData[] {
...
export const getLabeledValue: GetLabeledValue<FlattenOptionData[]> = (value, { options, prevValueMap, labelInValue, optionLabelProp }) => {
...
function getFilterFunction(optionFilterProp: string) {
  return (searchValue: string, option: OptionData | OptionGroupData) => {
    ...
    // console.log('[selector] getFilterFunction searchValue',value , lowerSearchText, option);
    // return lowerSearchText.startsWith(value) || value.includes(lowerSearchText);
    return (value && lowerSearchText.startsWith(value)) || value.includes(lowerSearchText);
  };
}
...
export function filterOptions(
  ...
  { optionFilterProp, filterOption }: { optionFilterProp: string; filterOption: boolean | FilterFunc<SelectOptionsType[number]> }
) {
 ...
 */
export function fillOptionsWithMissingValue(options: SelectOptionsType, value: DefaultValueType, optionLabelProp: string, labelInValue: boolean): SelectOptionsType {
  ...
  // Fill missing value
  values.forEach(item => {
    const val: RawValueType = labelInValue ? (item as LabelValueType).value : (item as RawValueType);
