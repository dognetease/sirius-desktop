...
import type { OptionsType as SelectOptionsType, FlattenOptionData as SelectFlattenOptionData, OptionData, RenderNode, OnActiveValue } from './interface';
  ...
  React.useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (!multiple && open && values.size === 1) {
        ...
        const index = memoFlattenOptions.findIndex(({ data }) => (data as OptionData).value === value);
        setActive(index);
    ...
    <>
      ...
      >
        {({ group, groupOption, data }, itemIndex) => {
          ...
          // Group
          if (group) {
            return <div className={classNames(itemPrefixCls, `${itemPrefixCls}-group`)}>{label !== undefined ? label : key}</div>;
          }

          const { disabled, value, title, children, style, className, ...otherProps } = data as OptionData;
          ...
          let optionTitle = typeof content === 'string' || typeof content === 'number' ? content.toString() : undefined;
          if (title !== undefined) {
            optionTitle = title;
          }
