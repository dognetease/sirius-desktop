...
var isEdmProd = location ? location.hostname === 'waimao.office.163.com' : false;
...
versionCachesUrls = versionCachesUrls
  .map(function (url) {
    ...
    if (fullUrl.includes('https://waimao.office.163.com')) {
      fullUrl = fullUrl.replace('https://waimao.office.163.com', 'https://waimao-cdn.office.163.com');
    }
    return fullUrl;
  })
  .filter(function (url) {
...
self.addEventListener('fetch', function (event) {
  ...
  if (versionCacheMap && versionCacheMap[url]) {
    cacheConfigItem = {
      ...
      ignoreSearch: true,
      credentials: isEdmProd ? 'omit' : 'include',
    };
  }
