...
for (let i = 0; i < cnStr.length; ++i) {
  var cnChar = cnStr.charAt(i);
  var twChar = twStr.charAt(i);
  if (!map.has(cnChar)) {
    map.set(cnChar, twChar);
  }
}

function traditionlize_TW(cc) {
  var str = '';
  for (var i = 0; i < cc.length; i++) {
    var currentChar = cc.charAt(i);
    if (map.has(currentChar)) {
      str += map.get(currentChar);
    } else {
      str += currentChar;
    }
  }
...
function translateJsonObject(obj) {
  ...
  for (let i = 0; i < keys.length; ++i) {
    ...
    newObj[currKey] = traditionlize_TW(currVal);
  }
  return newObj;
}
...
function translteFile(sourceFile, targetFilePath) {
  const zhJsonStr = fs.readFileSync(getAbsoluteFilePath(sourceFile), { encoding: 'utf-8' });
  const zhObj = JSON.parse(zhJsonStr);
  const twObj = translateJsonObject(zhObj);
  fs.writeFileSync(getAbsoluteFilePath(targetFilePath), JSON.stringify(twObj));
}

// translteFile('./global_label/zh.json', './global_label/zh-trad.json');
translteFile('./global_label/waimao/zh.json', './global_label/waimao/zh-trad.json');
// translteFile('./global_label/lingxi-api/zh.json', './global_label/lingxi-api/zh-trad.json');
// translteFile('./global_label/lingxi-api/zh.json', './global_label/lingxi-api/zh-trad.json');
// translteFile('./global_label/lingxi-preview-img/zh.json', './global_label/lingxi-preview-img/zh-trad.json');
// translteFile('./global_label/yingxiao/zh.json', './global_label/yingxiao/zh-trad.json');
