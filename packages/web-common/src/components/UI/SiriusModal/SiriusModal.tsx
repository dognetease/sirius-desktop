...
// import { ModalProps, ModalFuncProps } from './types';
...
import { testPathMatch } from '../utils/capi';
import { inEdm, isElectron, isWebWmEntry } from '../utils/cenv';
import { SIDE_BAR_WIDTH, TOOL_BAR_HEIGHT } from '../utils/constant';
...
export const modalProps: SiriusModalProps = {
  ...
  wrapClassName: `${!testPathMatch('/') ? styles.modalWrapGlobal : !isWebWmEntry() ? styles.modalWrap : ''}`,
  // systemApi.isWebWmEntry()在非外贸和外贸测试环境为false，外贸预发及线上为true
  maskStyle: {
    background: 'rgba(0, 0, 0, 0.5)',
    left: `${!testPathMatch('/') ? 0 : !isWebWmEntry() ? SIDE_BAR_WIDTH : 0}px`,
    top: `${!testPathMatch('/') ? 0 : !inEdm() && !isElectron() ? TOOL_BAR_HEIGHT : 0}px`,
  },
