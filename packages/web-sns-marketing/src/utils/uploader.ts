...
import { apis, apiHolder, ApiRequestConfig, SnsMarketingApi, SnsMarketingUploadInfo } from 'api';
...
const getUploadToken = (options: { fileName: string }) => {
  return snsMarketingApi.getUploadToken({ fileName: options.fileName });
};
...
export class Uploader extends EventEmitter {
  ...
  uploadInfo?: SnsMarketingUploadInfo;
  ...
  startUpload() {
    ...
    getUploadToken(option)
      .then(data => {
        this.uploadInfo = data as unknown as SnsMarketingUploadInfo;
      ...
      .then(ret => {
        if (ret.status === 200) {
          if (isComplete) {
            ...
            (this.uploadInfo as SnsMarketingUploadInfo).context = data.context;
