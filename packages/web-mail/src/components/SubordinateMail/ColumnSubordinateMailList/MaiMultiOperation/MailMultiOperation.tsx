...
const MailMultiOperation: React.FC<Props> = ({ cancelFn }) => {
  // 全选/半选
  ...
  // 邮件列表-当前选中的邮件id
  const [selectedMail] = useState2SubordinateSlice('selectedMailId');
  ...
  const mailIdList = useMemo(() => {
    return mailDataList.filter(mail => mail?.taskInfo?.status !== TASK_MAIL_STATUS.PROCESSING).map(mail => mail.id);
  }, [mailDataList]);
  ...
  return (
    ...
    >
      ...
      {/* 批量操作按钮,通栏下展示 */}
      {!isLeftRight && <MailMultiOperationBtns size="mini" sliceId={sliceId} />}
    </div>
  );
};
