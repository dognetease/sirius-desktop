...
import { HotKeys, KeyMap } from '../../common/library/HotKeys/react-hotkeys';
...
const HotKeysHoc = (ListElement: React.Component, customEventOrign?: React.Component) => props => {
  const {
    mailDataList,
    onSelect,
    onUnSelect,
    ...
    if (activeIds != null && Array.isArray(activeIds)) {
      ...
      if (!(activeIds.length === _activeIds.current.length && activeIds.every(id => idIsActive(id)))) {
        ...
        if (activeIds.length == 0) {
           ...
           */
          _activeIds.current = activeIds;
          if (activeIds.length == 1) {
            // 部分操作后选中焦点的转移，快捷键内部指针也得跟随
            setShiftActiveId(activeIds[0]);
            setShiftCurId(activeIds[0]);
          } else if (id2Index(shiftActiveId) == null || id2Index(shiftCurId) == null) {
  ...
  const handleItemSelect = (ids: string[], _activeIds) => {
    onSelect && onSelect(ids, _activeIds);
  };

  const handleItenUnSelect = (ids: string[], _activeIds) => {
    onUnSelect && onUnSelect(ids, _activeIds);
  };

  const handleItemonContextmenu = mailList => {
    onContextmenu && onContextmenu(mailList);
  };

  const handleItemActive = id => {
    const index = id2Index(id);
    const data = mailDataList[index];
    onActive && onActive(id, index, data, _activeIds.current);
  };
