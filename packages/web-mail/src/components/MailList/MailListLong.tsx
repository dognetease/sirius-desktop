...
import MailCardLong, { getCardHeight } from '../../common/components/vlistCards/MailCard/MailCardLong';
...
import HotKeysLongMult from '../../common/components/HotKeys/HotKeysLongMult';
import { ModuleHotKeyEvent } from '../ListHotKeys/moduleHotKey';
import { getMailKey, scanMailsSetTimeGradients, isMainAccount, folderIdIsContact, systemIsWindow } from '../../util';
...
const MailListLong: React.FC<any> = (props, ref) => {
  const {
    // 事件
    onContextMenu,
    onDragStart,
    ...
    onListModelChange = defaultCallback,
    /**
    ...
    realListPager = null,
    onRealListScroll,
    useLeftRightHotKey = false,
    isRefresh,
    onContentInsufficientHeight,
  } = props;
  // const [selectedKeys, setSelectedKeys] = useState2RM('selectedKeys', 'doUpdateSelectedKey', forCustomer);
  // 邮件列表-上部-二级tab选中
  // const [selected] = useState2RM('mailListStateTab', 'doUpdateMailListStateTab', forCustomer);
  const isCorpMail = useMemo(() => {
    return forCustomer && systemApi.getIsCorpMailMode();
  }, [forCustomer]);
  // const [listModel, setListModel] = useState2RM('defaultMailListSelectedModel', 'doUpdateMailListSelectedModel', forCustomer);
  ...
  const ListHotKeys = useMemo(() => {
    if (useLeftRightHotKey) {
      return HotKeys(MailCardList, ModuleHotKeyEvent);
    }
    return HotKeysLongMult(MailCardList, ModuleHotKeyEvent);
  }, [useLeftRightHotKey]);
  // 判断是否支持拖拽
  ...
  const CustomMailCard = useCallback(
    ...
    [selectedKeys?.id, tagName, listModel, mailCardDesc, cardFromTitle, showAvator, showCheckbox, CardOperRedFlagWrap]
  );
  ...
  const mailGroupDecorate = useMemo(() => {
    ...
    if (isMainAccount(selectedKeys.accountId)) {
      //   // 任务邮件
      //   // 注意：通栏的显示样式与分栏不同，不可直接合并
      //   list.push((data, preIndex = 0) => {
      ...
      //         return [
      //           {
      //             element: <div className="mail-list-item-top-time">{getIn18Text('RENWU')}</div>,
      //             index: preIndex,
      //             height: 27,
      //             position: 'top',
      //             fixed: {
      ...
      //           {
      //             element: (
      //               <div className="mail-list-long-topline" onClick={goTaskMailBox}>
      //                 {data[0].entry.taskNum &&
      //                   (data[0].entry.taskNum <= 2 ? (
      //                     <span className="content">{getIn18Text('YISHANGWEIJINXING')}</span>
      //                   ) : (
      //                     <span className="mail-list-topline-more content">
      //                       {getIn18Text('HAIYOU')}
      //                       {wordSeparator}
      //                       <span className="text-blue"> {data[0].entry.taskNum - 2} </span>
      //                       {wordSeparator}
      //                       {getIn18Text('GERENWU')}
      //                       <span className="icon">
      //                         <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      //                           <path d="M6 4L10 8L6 12" stroke="#A8AAAD" stroke-width="1.2" stroke-linejoin="round" />
      //                         </svg>
      //                       </span>
      //                     </span>
      //                   ))}
      //               </div>
      //             ),
      //             index: taskSum - 1, // 插入位置 最后面的置顶的下面
      //             height: 49, // 总高度
      //             position: 'bottom'
      //           }
      //         ];
        ...
        list.push((data, preIndex) => {
          if (data && data.length) {
            ...
            if (hasTop) {
              return [
                {
                  element: <div className="mail-list-item-top-time"> {getIn18Text('ZHIDING')}</div>,
                  index: preIndex,
                  height: 27,
                  position: 'top',
                  fixed: {
  ...
  return (
    <div className={'mail-list' + (!useRealList ? ' sirius-no-drag' : '')}>
      ...
      }, [listLoading, useRealList, isSearching])}
    </div>
  );
};
export default React.forwardRef(MailListLong);
