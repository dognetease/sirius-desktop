...
const HotKey = (ListCom: React.FC<any>, CustomEventOrign?: React.FC<any>) => (props: CardListComProps<any>) => {
  const {
    data = [],
      ...
      if (!(activeId.length === _activeIds.current.length && activeId.every(id => idIsActive(id)))) {
        ...
        if (activeId.length == 0) {
          ...
          if (activeId.length == 1) {
            ...
            _setListModel(LIST_MODEL.SINGLE);
            onListModelChange && onListModelChange(LIST_MODEL.SINGLE);
          } else {
            _setListModel(LIST_MODEL.MULTIPLE);
  ...
  const handleItemSelect = (ids: string[], _activeIds, curId) => {
    onSelect && onSelect(ids, _activeIds, curId);
  };

  const handleItenUnSelect = (ids: string[], _activeIds, curId) => {
    onUnSelect && onUnSelect(ids, _activeIds, curId);
  };
