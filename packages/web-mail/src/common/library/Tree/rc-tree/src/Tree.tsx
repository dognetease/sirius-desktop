...
import { TreeContext, NodeMouseEventHandler, NodeDragEventHandler, NodeDragEventParams, NodeMouseEventParams } from './contextTypes';
import {
  getDataAndAria,
  getDragChildrenKeys,
  parseCheckedKeys,
...
export interface TreeProps {
  ...
  dropIndicatorRender?: (props: { dropPosition: -1 | 0 | 1; dropLevelOffset: number; indent: number; prefixCls: string; direction: Direction }) => React.ReactNode;
...
class Tree extends React.Component<TreeProps, TreeState> {
  ...
  static getDerivedStateFromProps(props: TreeProps, prevState: TreeState) {
    ...
    if (needSync('expandedKeys') || (prevProps && needSync('autoExpandParent'))) {
      newState.expandedKeys =
        props.autoExpandParent || (!prevProps && props.defaultExpandParent) ? conductExpandParent(props.expandedKeys, keyEntities) : props.expandedKeys;
      ...
      newState.expandedKeys =
        props.autoExpandParent || props.defaultExpandParent ? conductExpandParent(props.defaultExpandedKeys, keyEntities) : props.defaultExpandedKeys;
    }
  ...
  onNodeDragEnter = (event: React.DragEvent<HTMLDivElement>, node: NodeInstance) => {
    ...
    const { onDragEnter, onExpand, allowDrop, direction, dragModel } = this.props;
    const { pos, eventKey } = node.props;
    const isOuterDrag = dragModel === 'drag';
    const { dragNode } = this;
    ...
    const {
      ...
      dropTargetPos,
      dropAllowed,
      // 正在over的node元素，一般与响应的元素key相同
      dragOverNodeKey,
    } = isOuterDrag
      ? outDropCalcDropPosition(event, dragNode, node, indent, this.dragStartMousePosition, allowDrop, flattenNodes, keyEntities, expandedKeys, direction)
      : calcDropPosition(event, dragNode, node, indent, this.dragStartMousePosition, allowDrop, flattenNodes, keyEntities, expandedKeys, direction);

    if (
      (!dragNode && !isOuterDrag) ||
    ...
    if (isOuterDrag || dragNode?.props?.eventKey !== node.props.eventKey) {
  ...
  onNodeDragOver = (event: React.DragEvent<HTMLDivElement>, node: NodeInstance) => {
    ...
    const { onDragOver, allowDrop, direction, dragModel } = this.props;
    const isOuterDrag = dragModel == 'drag';
    const { dragNode } = this;

    const { dropPosition, dropLevelOffset, dropTargetKey, dropContainerKey, dropAllowed, dropTargetPos, dragOverNodeKey } = isOuterDrag
      ? outDropCalcDropPosition(event, dragNode, node, indent, this.dragStartMousePosition, allowDrop, flattenNodes, keyEntities, expandedKeys, direction)
      : calcDropPosition(event, dragNode, node, indent, this.dragStartMousePosition, allowDrop, flattenNodes, keyEntities, expandedKeys, direction);

    if ((!dragNode && !isOuterDrag) || dragChildrenKeys.indexOf(dropTargetKey) !== -1 || !dropAllowed) {
      // don't allow drop inside its children
      // don't allow drop when drop is not allowed caculated by calcDropPosition
      return;
    }
    ...
    if (dragNode && dragNode?.props?.eventKey === dropTargetKey && dropLevelOffset === 0) {
      ...
      this.setState({
        ...
        dragOuterOver: isOuterDrag ? dropTargetKey : null,
        dropContainerKey,
        dropTargetPos,
        dropAllowed,
        dragOverNodeKey,
  ...
  onNodeDrop = (event: React.DragEvent<HTMLDivElement>, node, outsideTree: boolean = false) => {
    ...
    const { onDrop, dragModel } = this.props;
    const isOuterDrag = dragModel === 'drag';
    ...
    const dropResult = {
      ...
      dragNodesKeys: this.dragNode ? [this.dragNode.props.eventKey].concat(dragChildrenKeys) : null,
  ...
  onNodeCheck = (e: React.MouseEvent<HTMLSpanElement>, treeNode: EventDataNode, checked: boolean) => {
   ...
   */
  setUncontrolledState = (state: Partial<TreeState>, atomic: boolean = false, forceState: Partial<TreeState> | null = null) => {
    if (this.destroyed) {
      return;
    }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <NodeList
            // dragModel={this.props.dragModel}
            ref={this.listRef}
            prefixCls={prefixCls}
            style={style}
            data={flattenNodes}
