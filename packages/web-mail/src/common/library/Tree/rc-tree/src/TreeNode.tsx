...
class InternalTreeNode extends React.Component<InternalTreeNodeProps, TreeNodeState> {
  ...
  onDragStart = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      // 获取当期那元素的宽度
      const offsetWidth = e.currentTarget.clientWidth >> (1 + 20);
      const offsetHeight = e.currentTarget.clientHeight;
      e.dataTransfer.setDragImage(e.currentTarget, -offsetWidth, -5);
    } catch (error) {
      // empty
    }
  };
  ...
  renderSelector = () => {
    ...
    return (
      ...
      >
        {
          // 扩大菜单折叠的点击区域，图标同样触发展开事事件，屏蔽掉图标的点击事件
        }
        <span
          onClick={(e: any) => {
            e.stopPropagation();
  ...
  renderDropIndicator = () => {
    ...
    const {
      context: { dragModel, draggable, dropLevelOffset, dropPosition, prefixCls, indent, dropIndicatorRender, dragOverNodeKey, direction },
    } = this.props;
    ...
    return showIndicator && !isOuterDrag
      ? dropIndicatorRender({
          dropPosition,
          dropLevelOffset,
          indent,
  ...
  render() {
    ...
    const {
      context: { dragModel, prefixCls, filterTreeNode, draggable, keyEntities, dropContainerKey, dropTargetKey },
    } = this.props;
    const isOuterDrag = dragModel == 'drag';
    const disabled = this.isDisabled();
    ...
    const mergedDraggable = (typeof draggable === 'function' ? draggable(data) : draggable) || isOuterDrag;
    return (
      <div
        ref={domRef}
        className={classNames(className, `${prefixCls}-treenode`, {
          ...
          'drop-target': dropTargetKey === eventKey && !isOuterDrag,
          'drop-container': dropContainerKey === eventKey && !isOuterDrag,
          'drag-over': !disabled && dragOver && !isOuterDrag,
          'drag-over-gap-top': !disabled && dragOverGapTop && !isOuterDrag,
          'drag-over-gap-bottom': !disabled && dragOverGapBottom && !isOuterDrag,
          'filter-node': filterTreeNode && filterTreeNode(convertNodePropsToEventData(this.props)),
          'drag-outer-over': !disabled && (dragOver || dragOverGapTop || dragOverGapBottom || dropContainerKey === eventKey) && isOuterDrag,
        })}
        style={style}
...
const ContextTreeNode: React.FC<TreeNodeProps> = props => <TreeContext.Consumer>{context => <InternalTreeNode {...props} context={context} />}</TreeContext.Consumer>;
