...
export function calcDropPosition(
  ...
  if (clientY < top + height / 2) {
    ...
    const nodeIndex = flattenedNodes.findIndex(flattenedNode => flattenedNode.data.key === abstractDropNodeEntity.key);
...
// 外部拖拽的偏移位置计算
export function outDropCalcDropPosition(
  event: React.MouseEvent,
  dragNode: NodeInstance,
  targetNode: NodeInstance,
  indent: number,
...
export function convertDataToTree(treeData: DataNode[], processor?: { processProps: (prop: DataNode) => any }): NodeElement[] {
  if (!treeData) return [];
